...
import AddPurchasing from './CattlePurchasingItem/AddPurchasing';
import CattlePurchasingTable from './CattlePurchasingItem/PurchasingTable';
import CattlePurchasingStats from './CattlePurchasingItem/PurchasingStats';
import PrintPO from '@/app/(Main)/Components/Purchasing/PrintPo';
import SearchInput from '@/app/(Main)/Components/SeachInput';
import CattleDeliveryOrderTable, {
  AddDOModal,
  DODetailModal,
  CreatePOFromDOModal,
...
const Purchasing = () => {
  ...
  const fetchDOs = useCallback(async () => {
    setDoLoading(true);
    try {
      const res  = await fetch('/api/cattle/delivery-order');
      const data = await res.json();
  ...
  const handleStatusUpdate = async (id, newStatus) => {
    const res = await fetch(`/api/cattle/purchasing/${id}/approve`, {
  ...
  const handleBulkStatusUpdate = async (ids, newStatus) => {
    const res = await fetch('/api/cattle/purchasing/bulk-status', {
      method: 'PATCH', headers: { 'Content-Type': 'application/json' },
  ...
  const deletePurchasing = async (id) => {
    ...
    const res = await fetch(`/api/cattle/purchasing/${id}`, { method: 'DELETE' });
  ...
  const handleViewDO = async (do_) => {
    ...
    try {
      const res  = await fetch(`/api/cattle/delivery-order/${do_.id}`);
      const data = await res.json();
      setSelectedDO(data);
  ...
  const handleDeleteDO = async (id) => {
    ...
    const res = await fetch(`/api/cattle/delivery-order/${id}`, { method: 'DELETE' });
    if (res.ok) fetchDOs();
  ...
  const handleApproveDO = async () => {
    try {
      const res = await fetch(`/api/cattle/delivery-order/${selectedDO.id}`, {
        method: 'PATCH', 
      ...
      if (res.ok) { 
        ...
        alert(err.message || 'Gagal approve DO');
      }
    } catch (err) {
      alert(err.message);
    }
  ...
  const notes = prompt('Alasan penolakan (opsional):');
    try {
      ...
      if (res.ok) { 
        ...
        const err = await res.json();
        alert(err.message || 'Gagal reject DO');
      }
    } catch (err) {
      alert(err.message);
    }
  ...
  const filteredPOs = requests.filter(req => {
    ...
    return (req.noPO || '').toLowerCase().includes(q) ||
      ...
      (req.items?.[0]?.jenisSapi || '').toLowerCase().includes(q) ||
  ...
  const filteredDOs = deliveryOrders.filter(do_ => {
    const matchSearch =
      ...
      do_.items?.some(i => (i.jenisSapi || '').toLowerCase().includes(doSearch.toLowerCase()));
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa]">
      ...
      <div className="p-3 md:p-6 lg:p-8 space-y-4 md:space-y-6 text-gray-800 print:hidden">
        ...
        {/* PO Stats */}
        <CattlePurchasingStats requests={requests} />
        ...
        <div className="bg-white rounded-[24px] md:rounded-[32px] shadow-sm border border-gray-100 overflow-hidden flex flex-col">
          ...
          <div className="relative overflow-x-auto">
            {activeTab === 'po' ? (
              <>
                <CattlePurchasingTable
                  data={filteredPOs}
                  onStatusUpdate={handleStatusUpdate}
                  onDelete={deletePurchasing}
                  onBulkStatusUpdate={handleBulkStatusUpdate}
                  onViewDetail={() => {}} // Add detail view handler if needed
                />
