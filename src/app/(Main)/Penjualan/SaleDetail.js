            ...
            <div className="bg-white w-full max-w-2xl rounded-4xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-8">
                ...
                <div className="p-6 space-y-8">
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {sale.items?.map((item, idx) => (
                                        <tr key={idx}>
                                            ...
                                            <td className="px-4 py-3 text-sm font-black text-gray-900 text-right">Rp {item.price.toLocaleString('id-ID')}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="flex justify-between items-center p-6 bg-[#8da070] rounded-3xl text-white">
                        ...
                        {sale.status === 'PENDING' && (
                            <div className="grid grid-cols-2 gap-3">
                                <button 
                                    ...
                                    className="flex items-center justify-center gap-2 py-4 bg-green-500 hover:bg-green-600 text-white rounded-2xl font-black uppercase text-xs transition-all shadow-lg shadow-green-500/20"
                                >
                                    <CheckCircle size={18}/> Selesaikan Pesanan
                                </button>
                                <button 
                                    onClick={() => onStatusUpdate(sale.id, 'CANCELLED')}
                                    className="flex items-center justify-center gap-2 py-4 bg-red-500 hover:bg-red-600 text-white rounded-2xl font-black uppercase text-xs transition-all shadow-lg shadow-red-500/20"
                                >
                        ...
                        {sale.status === 'COMPLETED' && (
                            <div className="p-4 bg-green-50 border border-green-100 rounded-2xl flex items-center gap-3 text-green-700">
                                <CheckCircle size={20}/>
                                <p className="text-xs font-bold uppercase tracking-tight">Transaksi ini telah selesai dan dibukukan.</p>
                            </div>
                        )}
                    </div>
                </div>
