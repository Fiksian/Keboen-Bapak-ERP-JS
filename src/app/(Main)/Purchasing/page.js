...
import { 
  ...
  Trash2, Building2, Hash, CalendarDays, User, ShieldCheck, Truck
} from 'lucide-react';
  ...
  return (
    <div className="p-6 bg-[#f8f9fa] min-h-full space-y-8 animate-in fade-in duration-500 text-gray-800">
      ...
      <div className="bg-white rounded-[32px] p-8 shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl shadow-xl shadow-blue-200 transition-all active:scale-95 font-bold text-sm uppercase tracking-tighter italic">
              ...
              {requests.length > 0 ? requests.map((req) => {
                ...
                return (
                  <tr key={req.id} className="hover:bg-blue-50/10 transition-colors group">
                    ...
                    {/* Column 3: STAKEHOLDERS (Req, Acc, Rec) */}
                    <td className="px-6 py-6">
                      <div className="flex flex-col gap-1.5">
                        ...
                        {/* Receiver (New!) */}
                        {req.isReceived && (
                          <div className="flex items-center gap-2 text-[9px] font-black bg-blue-600 text-white px-2 py-1 rounded-lg shadow-sm animate-in fade-in zoom-in duration-300">
                            <Truck size={10} />
                      ...
                      <div className="flex justify-end gap-3 items-center">
                        {req.isReceived ? (
                          <div className="flex flex-col items-end gap-1">
                            <div className="flex items-center gap-1.5 text-blue-600 font-black text-[9px] bg-blue-50 px-4 py-2 rounded-xl border border-blue-100 uppercase italic shadow-sm">
                              <PackageCheck size={14} /> IN WAREHOUSE
                            </div>
                            <span className="text-[8px] font-bold text-gray-400 uppercase tracking-tighter mr-1 italic">
                              Closed Transaction
                            </span>
                          </div>
                        ) : (
                          <div className="flex gap-2">
                            {req.status === 'APPROVED' ? (
                              <button 
                                ...
                                className="flex items-center gap-1.5 px-4 py-2 text-orange-500 hover:bg-orange-50 rounded-xl transition-all text-[9px] font-black border border-orange-100 uppercase italic cursor-pointer"
                              >
                              ...
                              <>
                                <button 
                                  ...
                                  className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white text-[9px] font-black rounded-xl transition-all shadow-sm uppercase italic cursor-pointer"
                                >
                                  APPROVE
                                </button>
                                <button 
                            ...
                            <button 
                              ...
                              className="p-2 text-gray-300 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all border border-transparent hover:border-red-100 cursor-pointer"
                            >
                              <Trash2 size={18} />
                            </button>
                          </div>
