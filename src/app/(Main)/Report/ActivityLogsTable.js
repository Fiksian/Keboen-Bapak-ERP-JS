...
const ActivityLogsTable = ({ logs = [], showViewAll = true }) => {
  const limitedLogs = logs.slice(0, 8);
    ...
    <div className="bg-white rounded-[24px] md:rounded-[32px] shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="w-full">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50 bg-gray-50/50">
                <th className="px-8 py-5 w-[25%]">Activity & Item</th>
                <th className="px-6 py-5 w-[15%]">Category</th>
                <th className="px-6 py-5 w-[20%] text-center">User Trace</th>
                <th className="px-8 py-5 w-[40%] text-right">Reference Notes</th>
              </tr>
            </thead>
          </table>
        </div>
        ...
        <div className="divide-y divide-gray-50">
          {limitedLogs.length > 0 ? limitedLogs.map((log, idx) => (
            <div 
              key={idx} 
