...
const Report = () => {
  ...
  if (error) return (
    <div className="p-10 text-center bg-white rounded-[32px] border border-red-100 m-6 shadow-xl shadow-red-50/50">
      ...
      <button onClick={() => window.location.reload()}
        className="mt-6 px-6 py-2 bg-gray-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest italic active:scale-95 transition-all">
        Try Again
      </button>
    </div>
  );

  const handleExcelExport = async (period) => {
  ...
  return (
    <div className="p-4 md:p-8 bg-[#f8f9fa] min-h-full space-y-6 md:space-y-10 animate-in fade-in duration-500">
        ...
        <div className="flex flex-row gap-2 w-full lg:w-auto relative">
          ...
          <div className="relative flex-1 lg:flex-none" onClick={(e) => e.stopPropagation()}>
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-6 py-3.5 bg-[#8da070] text-white rounded-2xl text-[10px] font-black hover:bg-[#7a8c5f] shadow-xl shadow-[#8da070]/20 uppercase tracking-wider transition-all active:scale-95 disabled:opacity-60"
            >
              {exportLoading
                ? <Loader2 size={14} className="animate-spin" />
                : <Download size={14} strokeWidth={3} />
              }
              {exportLoading ? 'Menyiapkan...' : 'Export Data'}
              {!exportLoading && (
            ...
            {showExportMenu && (
              <div className="absolute right-0 mt-3 w-64 bg-white border border-gray-100 rounded-[24px] shadow-2xl z-[100] overflow-hidden animate-in slide-in-from-top-2">
                <div className="p-4 border-b border-gray-50 bg-gray-50/50">
                  <span className="text-[8px] font-black text-gray-400 uppercase tracking-[0.2em]">Export Format & Periode</span>
                </div>
                <div className="p-2">
                  <p className="px-4 pt-2 pb-1 text-[8px] font-black text-[#8da070] uppercase tracking-widest">Excel (.xlsx)</p>
                  {['Daily', 'Weekly', 'Monthly'].map((period, i) => {
                    const icons = [TrendingUp, Package, HistoryIcon];
                    const Icon = icons[i];
                    return (
                      <button
                        key={period}
                        onClick={() => handleExcelExport(period)}
                        className="w-full text-left px-4 py-3 text-[10px] font-black text-gray-700 hover:bg-[#8da070]/5 hover:text-[#8da070] rounded-xl transition-colors uppercase italic flex items-center justify-between group"
                      >
                        <span className="flex items-center gap-2">
                          <FileSpreadsheet size={12} className="text-green-600" />
                          {period} Report
                        </span>
                        <Icon size={12} className="opacity-0 group-hover:opacity-100" />
                      </button>
                    );
                  })}
                  ...
                  <p className="px-4 pt-1 pb-1 text-[8px] font-black text-blue-500 uppercase tracking-widest">CSV (Data Mentah)</p>
                  <button
                    onClick={handleCSVExport}
                    className="w-full text-left px-4 py-3 text-[10px] font-black text-gray-700 hover:bg-blue-50 hover:text-blue-600 rounded-xl transition-colors uppercase italic flex items-center justify-between group"
                  >
                    <span className="flex items-center gap-2">
                      <FileText size={12} className="text-blue-500" />
                      Download CSV (Kedatangan)
                    </span>
                  ...
                  <button
                    onClick={() => handleExcelExport(range)}
                    className="w-full text-left px-4 py-3 text-[10px] font-black text-[#8da070] hover:bg-[#8da070]/10 rounded-xl transition-colors uppercase italic flex items-center justify-between group"
                  >
                    <span className="flex items-center gap-2">
                      <Download size={12} />
                      Download Current ({range})
                    </span>
                  </button>
                </div>
              </div>
      ...
      {/* Inventory Alert + Production Chart */}
